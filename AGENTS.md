...
(genera contenido). Detalle de producto y arquitectura: ver `agente_log/`.

## 📌 Empieza aquí: registro de sesiones (`agente_log/`)

**Antes de tocar nada, lee el handoff más reciente en `agente_log/`.** Es la
...
1. **Texto canónico.** El string de `TextLoader.load()` es idéntico al que
   renderiza el frontend. Una sola normalización. Nunca dos.
...
- **Antes de editar `contracts.py` o `schemas.py`** (superficie compartida):
  avisa y escribe el cambio en `agente_log/`. Un rename de campo se coordina en
